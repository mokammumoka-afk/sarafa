...
const KEYS = ['company_info', 'deposit_limits', 'withdraw_limits', 'trade_limits', 'p2p_settings'];
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-surface-900 border border-white/5 rounded-2xl p-5 space-y-3 col-span-2">
          <div className="flex items-center justify-between">
            <p className="font-medium">سوق P2P <span className="text-xs text-zinc-500">(قيد التحضير)</span></p>
            <button
              onClick={() => { update('p2p_settings', 'enabled', !settings.p2p_settings?.enabled); }}
              className={`w-11 h-6 rounded-full relative transition-colors ${settings.p2p_settings?.enabled ? 'bg-accent-400' : 'bg-zinc-700'}`}
            >
              <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full transition-transform ${settings.p2p_settings?.enabled ? 'right-0.5' : 'left-0.5'}`} />
            </button>
          </div>
