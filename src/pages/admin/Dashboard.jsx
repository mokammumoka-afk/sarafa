...
import { Users, Wallet, Gem, Clock } from 'lucide-react';
import { supabase } from '../../lib/supabase';
import { formatLYD, formatUSDT, formatDate, TYPE_LABELS } from '../../lib/utils';

const PIE_COLORS = ['#D4AF37', '#10B981', '#EF4444', '#F59E0B'];
...
export default function AdminDashboard() {
  ...
  async function load() {
    setLoading(true);
    const [{ count: userCount }, { data: balances }, { count: pendingCount }, { data: recentTxns }] = await Promise.all([
    ...
    setStats({ userCount: userCount || 0, totalLyd, totalUsdt, pendingCount: pendingCount || 0 });
    setRecent(recentTxns || []);

    // Last 30 days transaction volume by day
    const since = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString();
    const { data: monthTxns } = await supabase.from('transactions').select('type, amount_lyd, created_at').gte('created_at', since);
    const byDay = {};
    (monthTxns || []).forEach((t) => {
      const day = t.created_at.slice(5, 10);
      byDay[day] = byDay[day] || { day, deposit: 0, buy_usdt: 0, sell_usdt: 0, withdraw: 0 };
      byDay[day][t.type] = (byDay[day][t.type] || 0) + Number(t.amount_lyd || 0);
    });
    setChartData(Object.values(byDay));

    const today = new Date().toISOString().slice(0, 10);
    const todayTxns = (monthTxns || []).filter((t) => t.created_at.startsWith(today));
    const counts = {};
    todayTxns.forEach((t) => { counts[t.type] = (counts[t.type] || 0) + 1; });
  ...
  const Card = ({ icon: Icon, label, value }) => (
    <div className="bg-surface-900 border border-white/5 rounded-2xl p-4 flex items-center gap-3">
      <div className="w-10 h-10 rounded-xl bg-accent-400/10 flex items-center justify-center text-accent-400"><Icon size={18} /></div>
      <div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        <Card icon={Users} label="مستخدمين" value={stats.userCount} />
        <Card icon={Wallet} label="إجمالي الدينار" value={formatLYD(stats.totalLyd)} />
        <Card icon={Gem} label="إجمالي USDT" value={formatUSDT(stats.totalUsdt)} />
        <Card icon={Clock} label="معاملات معلقة" value={stats.pendingCount} />
      </div>
