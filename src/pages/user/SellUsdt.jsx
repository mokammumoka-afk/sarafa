...
export default function SellUsdt() {
  ...
  const { tradeLimits, companyInfo } = useSettings();
  ...
  const companyUsdtWallet = companyInfo.usdt_wallet || import.meta.env.VITE_COMPANY_USDT_WALLET || '—';
  ...
  function copyAddress() {
    navigator.clipboard.writeText(companyUsdtWallet);
    toast.success('تم نسخ العنوان');
  }

  function validate() {
    const n = Number(amount);
    if (!n || n <= 0) { toast.error('أدخل كمية صحيحة'); return false; }
    if (n > Number(profile?.balance_usdt || 0)) { toast.error('رصيد USDT غير كافٍ'); return false; }
    if (n < tradeLimits.min_sell_usdt) { toast.error(`الحد الأدنى للبيع هو ${tradeLimits.min_sell_usdt} USDT`); return false; }
    return true;
  }
  ...
  async function submit(e) {
    ...
    if (!txHash.trim()) return toast.error('يرجى إدخال هاش التحويل');
    setBusy(true);
    try {
  ...
  return (
    <div>
      <PageHeader title="بيع USDT" to="/wallet" />
      <div className="space-y-5">
        ...
        <div className="bg-surface-800 rounded-xl p-4 border border-white/5">
          ...
          <div className="flex items-center justify-between gap-2">
            <code className="text-xs text-accent-400 break-all ltr">{companyUsdtWallet}</code>
        ...
        <form onSubmit={submit} className="space-y-4">
          <label className="block">
            ...
            <span className="text-xs text-zinc-500 mt-1 block">الحد الأدنى للبيع: {tradeLimits.min_sell_usdt} USDT</span>
          </label>
