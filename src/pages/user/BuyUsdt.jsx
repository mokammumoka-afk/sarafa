...
export default function BuyUsdt() {
  ...
  const { tradeLimits } = useSettings();
  const navigate = useNavigate();
  const [amount, setAmount] = useState('');
  const [busy, setBusy] = useState(false);
  const [confirming, setConfirming] = useState(false);

  const usdtReceived = rate && amount ? Number(amount) / rate.sell_rate : 0;
  ...
  function validate() {
    ...
    if (usdtReceived < tradeLimits.min_buy_usdt) {
      toast.error(`الحد الأدنى للشراء هو ${tradeLimits.min_buy_usdt} USDT`);
      return false;
    }
    return true;
  }
  ...
  return (
    <div>
      <PageHeader title="شراء USDT" to="/wallet" />
      <div className="space-y-5">
        ...
        <label className="block">
          ...
          <span className="text-xs text-zinc-500 mt-1 block">
            الرصيد المتاح: {profile?.balance_lyd} د.ل — الحد الأدنى للشراء: {tradeLimits.min_buy_usdt} USDT
          </span>
        </label>
        ...
        <div className="bg-accent-400/10 border border-accent-400/20 rounded-xl p-4 text-center">
          ...
          <p className="font-heading font-bold text-2xl text-accent-400">{usdtReceived.toFixed(6)} USDT</p>
        </div>

        {!confirming ? (
          <button disabled={!amount || !rate} onClick={() => validate() && setConfirming(true)}
          ...
          <div className="space-y-3">
            <div className="bg-surface-800 rounded-xl p-4 text-sm space-y-1">
              ...
              <p>الكمية: <b>{usdtReceived.toFixed(6)} USDT</b></p>
